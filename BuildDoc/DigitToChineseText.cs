...
namespace BuildDoc
{
    ...
    /// 2. 人民币大写数字
    /// </summary>
    public class DigitToChineseText
    {
        ...
        public DigitToChineseText()
        {
            ...
            // 人民币专用数字组
            chnRMBText = new char[] { '零', '壹', '贰', '叁', '肆', '伍', '陆', '柒', '捌', '玖' };
            chnRMBDigit = new char[] { '拾', '佰', '仟', '万', '亿' };
            ...
            else // 有小数部分
            {
                ...
                if (strDigit.Length - 1 != indexOfPoint) // 如果“.”不是最后一个字符
                {
                    ...
                    if (bToRMB)
                    {
                        ...
                        {
                            strResult.Append("零元整");
                        }
                        // 如果结果字符串最后以"圆"结尾
                        else if ("元" == strResult.ToString().Substring(strResult.Length - 1, 1))
                        {
                            strResult.Append('整');
                        }
                    }
