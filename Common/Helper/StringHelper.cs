...
namespace Common.Helper
{
    public class StringHelper
    {
        ...
        {
            return Regex.IsMatch(strPhone, @"^((\+)?86|((\+)?86)?)0?1[3458]\d{9}$");
        }
        #endregion
        ...
        private static char[] forbidList = { ' ', '　' };
        public static bool FilePathHasInvalidChars(string path)
        {
            return (!string.IsNullOrEmpty(path) && path.IndexOfAny(Path.GetInvalidPathChars()) >= 0);
        }

        public static string ReplaceFilePathInvalidChars(string path)
        {
            string regexSearch = new string(Path.GetInvalidFileNameChars()) + new string(Path.GetInvalidPathChars()) + new string(forbidList);
