...
public static class FileHelper
{
    ...
    public static bool CreateFile(string filePath, byte[] buffer)
    {
        try
        {
            ...
            // 若文件存在且大小相同，则跳过
            if (IsExistFile(filePath) && file.Length == buffer.Length)
            {
                return true;
            }
        ...
        {
            LogHelper.Error(string.Format("保存名为[{0}]（大小：{1}）的文件已经存在", filePath, buffer.Length), ex);
            return false;
        }
        return true;
    }
    ...
    public static byte[] FileToBytes(string filePath)
    {
        ...
        {
            LogHelper.Error("读取文件出错：" + filePath, ex);
            return null;
        }
        finally
        {
